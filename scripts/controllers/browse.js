...
app.controller('BrowseController', function($scope, $routeParams, toaster, Task, Auth, Comment, Offer) {
    ...
    function setSelectedTask(task) {
      ...
      if($scope.signedIn()) {
        ...
        // check if the current login user has already made an offer for the selected task/errand
    ...
    $scope.cancelTask = function(taskId) {
      Task.cancelTask(taskId).then(function() {
        toaster.pop('success', 'This errand is cancelled successfully.');
      });
    };
    ...
    $scope.completeTask = function(taskId) {
      Task.completeTask(taskId).then(function() {
        toaster.pop('success', 'Congratulation! You have completed this errand');
      });
    };
});
